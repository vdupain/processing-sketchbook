...
String url="/Users/Vince/Pictures/winnie.gif"; // String contenant l'adresse internet de l'image à utiliser
...
void setup() { // fonction d'initialisation exécutée 1 fois au démarrage
  ...
  fill(0, 0, 255); // couleur remplissage RGB - noFill() si pas de rempli//ssage
  ...
  imgSrc=loadImage(url, "gif"); // crée un PImage contenant le fichier à partir adresse web
  ...
  //pipe = new GSPipeline(this, "filesrc location=/Users/vince/Movies/test.mov ! decodebin");  
  //pipe = new GSPipeline(this, "videotestsrc");  
  
  //pipe = new GSPipeline(this, "rtspsrc location=rtsp://192.168.1.150:8086 latency=1000 debug=1 rtp-blocksize=4096 short-header=true ! rtph264depay ! ffdec_h264 ! capsfilter caps=\"video/x-raw-yuv,width=720,height=480,depth=12,bpp=12\"");  
  pipe = new GSPipeline(this, "rtspsrc location=rtsp://localhost:5554/stream.sdp debug=1 ! decodebin");  
  //pipe = new GSPipeline(this, "playbin uri=rtsp://192.168.1.83:8080/videofeed"); 

  // The full pipeline that GSVideo passes to GStremeamer can be
  // obtained with the getPipeline() method: 
  println("Pipeline string:");
  println(pipe.getPipeline());
...
void  draw() { // fonction exécutée en boucle
  ...
  if (pipe.available()) {
    ...
    PImage cSmall = pipe.get();
    cSmall.resize(widthAR, heightAR);
